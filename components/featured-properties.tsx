...
async function getFeaturedProperties(): Promise<Property[]> {
  try {
    ...
    // Filtrer les biens premium côté client si la DB n'a pas encore de premium
    const props: Property[] = data.properties || []
    const premiums = props.filter((p) => p.premium)
    return premiums.length > 0 ? premiums.slice(0, 6) : props.slice(0, 6)
  } catch {
    return []
  }
}
        ...
        <div className="mb-10 flex flex-col items-start justify-between gap-4 md:flex-row md:items-end">
          <div>
            ...
            <p className="mt-2 max-w-md text-sm leading-relaxed text-muted-foreground">
              {"Découvrez nos biens immobiliers vérifiés et mis en avant pour leur qualité exceptionnelle."}
            </p>
          </div>
          <Link href="/recherche">
